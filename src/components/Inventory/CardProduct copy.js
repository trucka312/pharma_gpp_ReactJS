            ...
            result = products.map((data, index) => {
                return (
                    <div class="col-sm-3" style={{ marginBottom: "10px" }}>
                        <a data-toggle="modal" data-target="#modalDetails" onClick={() => this.handleInfoProduct(data.inventory, data.id, data.name, data.images, data.price, data.distributes, data.max_price, data.min_price, data.product_discount, data.quantity_in_stock, data.quantity_in_stock_with_distribute,data)}>
                            <div class="card">
