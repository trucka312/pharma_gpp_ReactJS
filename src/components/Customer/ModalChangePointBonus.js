...
class ModalChangePoint extends Component {
  ...
  handleSubmitChangePoint = (e) => {
    ...
    if (coin.point === "" || Number(coin.point) === 0) {
      this.setState({ errorPoint: "Vui lòng nhập số điểm!" });
      return;
    }
    const newPoint = {
      ...coin,
      is_sub: isSub,
      diem_sp: Number(coin.point.toString().replace(/\./g, "")),
    };
    changePointBonusForCustomer(store_code, customerSelectedPoint.id, newPoint);
  };
...
const mapDispatchToProps = (dispatch) => {
  return {
    changePointBonusForCustomer: (store_code, idCustomer, data) => {
      dispatch(
        customerAction.changePointBonusForCustomer(store_code, idCustomer, data)
      );
    },
  };
};
