...
class ModalUpload extends Component {
  ...
  onSave= async (e) =>{
    ...
    if(typeof file !== "undefined" && file != "" && file != null )
    {
      ...
      this.props.uploadImgDiscount(fd)
  ...
  componentDidMount() {
    ...
    window.$('#file-discount').on('fileloaded', function (event, file) {
      _this.setState({ fileUpload: file })
    });
    window.$('#file-discount').on('fileremoved', function (event, id, index) {
      _this.setState({ fileUpload: null })
    });
    helper.loadFileInput("file-discount");
  }
  render() {
    return (
      <div
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    uploadImgDiscount: ( file) => {
      dispatch(discountAction.uploadImgDiscount(file));
    },
  };
};
