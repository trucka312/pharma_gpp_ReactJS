...
class TransferStock extends Component {
  ...
  changePage = (store_code, order_code) => {
    ...
    history.push(
      `/transfer_stocks/detail/${store_code}/${order_code}?tab-index=2&from=2&page=${page}&search=${searchValue}`
    );
  };
...
const mapStateToProps = (state) => {
  return {
    listTransferStock:
      state.importStockReducers.import_reducer.listTransferStockReceiver,
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    fetchAllTransferStockReceiver: (store_code, branch_id, page, params) => {
      dispatch(
        TransferStockAction.fetchAllTransferStockReceiver(
          store_code,
          branch_id,
          page,
          params
